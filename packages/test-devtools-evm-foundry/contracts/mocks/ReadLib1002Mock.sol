...
import { ILayerZeroTreasury } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/interfaces/ILayerZeroTreasury.sol";
...
import { ILayerZeroReadExecutor } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/interfaces/ILayerZeroReadExecutor.sol";
import { ILayerZeroReadDVN } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/uln/interfaces/ILayerZeroReadDVN.sol";
import { UlnOptions } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/uln/libs/UlnOptions.sol";
import { DVNOptions } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/uln/libs/DVNOptions.sol";
import { SafeCall } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/libs/SafeCall.sol";
import { MessageLibBase } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/MessageLibBase.sol";
import { ReadLibBase, ReadLibConfig } from "@layerzerolabs/lz-evm-messagelib-v2/contracts/uln/readlib/ReadLibBase.sol";

import { TestHelperOz5 } from "../TestHelperOz5.sol";

contract ReadLib1002Mock is ISendLib, ERC165, ReadLibBase, MessageLibBase {
    ...
    uint32 internal constant CONFIG_TYPE_READ_LIB_CONFIG = 1;
    ...
    function send(
        ...
        // Need to flip the srcEid and dstEid for the packet header on test packet
        bytes memory flippedPacket = PacketV1Codec.encode(
            Packet(
                _packet.nonce,
                _packet.dstEid,
                _packet.sender,
        ...
        testHelper.schedulePacket(flippedPacket, _options);
    ...
    function setConfig(address _oapp, SetConfigParam[] calldata _params) external onlyEndpoint {
        for (uint256 i = 0; i < _params.length; i++) {
            ...
            if (param.configType == CONFIG_TYPE_READ_LIB_CONFIG) {
    ...
    function getConfig(uint32 _eid, address _oapp, uint32 _configType) external view returns (bytes memory) {
        if (_configType == CONFIG_TYPE_READ_LIB_CONFIG) {
